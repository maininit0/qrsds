...
class AngleBasedHeadTiltServoController(Node):
    def __init__(self):
        ...
        self.THIGH_ANGLE_THRESHOLD = 3  # 角度变化阈值
        ...
        self.last_real_thigh_angle = 90  # 上一次检测到的真实大腿角度，初始化为90度
    ...
    def map_thigh_angle_to_servo_angle(self, real_angle):
        ...
        # 记录映射计算过程
        self.get_logger().info(f"大腿角度映射: 实际角度 {real_angle:.1f}° -> 舵机角度 {servo_angle}° (范围: {self.THIGH_MIN_ANGLE}-{self.THIGH_MAX_ANGLE})")
            ...
            if target.type == "person":
                for point_group in target.points:
                    if point_group.type == "body_kps" and len(point_group.point) >= 18:
                        ...
                        if has_head_points or has_arm_points or has_thigh_points:
                            ...
                            if has_thigh_points:
                                ...
                                if real_thigh_angle is not None:
                                    ...
                                    if angle_diff >= self.THIGH_ANGLE_THRESHOLD:
                                        ...
                                        self.get_logger().info(f"大腿角度变化不足: {angle_diff:.1f}° < {self.THIGH_ANGLE_THRESHOLD}°，保持当前舵机位置")
