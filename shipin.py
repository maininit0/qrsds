...
class FullBodyServoController(Node):
    def __init__(self):
        ...
        self.FOREARM_ANGLE_THRESHOLD = 4  # 角度变化阈值，超过此值才移动舵机
        
        # 右小臂舵机角度定义
        self.RIGHT_FOREARM_DEFAULT_ANGLE = 180  # 未检测到人时的默认角度
        self.RIGHT_FOREARM_ANGLE_THRESHOLD = 5  # 角度变化阈值
        
        # 左臂舵机角度定义
        self.ARM_DEFAULT_ANGLE = 180  # 未检测到人时的默认角度
        self.ARM_ANGLE_THRESHOLD = 4  # 角度变化阈值
        
        # 右臂舵机角度定义
        self.RIGHT_ARM_DEFAULT_ANGLE = 128  # 未检测到人时的默认角度
        self.RIGHT_ARM_ANGLE_THRESHOLD = 4  # 角度变化阈值
        self.RIGHT_ARM_MIN_ANGLE = 38       # 右臂舵机最小角度
        self.RIGHT_ARM_MAX_ANGLE = 218      # 右臂舵机最大角度
        
        # 左大腿舵机角度定义
        self.THIGH_DEFAULT_ANGLE = 200  # 未检测到人时的默认角度
        self.THIGH_MIN_ANGLE = 110      # 舵机最小角度（对应实际0度）
        self.THIGH_MAX_ANGLE = 200      # 舵机最大角度（对应实际90度）
        self.THIGH_ANGLE_THRESHOLD = 4  # 角度变化阈值
        ...
        self.RIGHT_THIGH_ANGLE_THRESHOLD = 4  # 角度变化阈值
        
        # 左小腿舵机角度定义
        self.LOWER_LEG_DEFAULT_ANGLE = 180  # 未检测到人时的默认角度
        self.LOWER_LEG_ANGLE_THRESHOLD = 4  # 角度变化阈值，超过此值才移动舵机
        
        # 右小腿舵机角度定义
        self.RIGHT_LOWER_LEG_DEFAULT_ANGLE = 180  # 未检测到人时的默认角度
        self.RIGHT_LOWER_LEG_ANGLE_THRESHOLD = 5  # 角度变化阈值
        ...
        self.current_right_forearm_angle = self.RIGHT_FOREARM_DEFAULT_ANGLE
        self.current_arm_angle = self.ARM_DEFAULT_ANGLE
        self.current_right_arm_angle = self.RIGHT_ARM_DEFAULT_ANGLE
        ...
        self.current_right_lower_leg_angle = self.RIGHT_LOWER_LEG_DEFAULT_ANGLE
        
        self.last_real_forearm_angle = 180  # 上一次检测到的真实小臂角度
        self.last_real_right_forearm_angle = 180  # 上一次检测到的真实右小臂角度
        self.last_real_arm_angle = 180      # 上一次检测到的真实手臂角度
        self.last_real_right_arm_angle = 90  # 上一次检测到的真实右臂角度
        self.last_real_thigh_angle = 90      # 上一次检测到的真实左大腿角度
        self.last_real_right_thigh_angle = 90  # 上一次检测到的真实右大腿角度
        self.last_real_lower_leg_angle = 180  # 上一次检测到的真实左小腿角度
        self.last_real_right_lower_leg_angle = 180  # 上一次检测到的真实右小腿角度
    ...
    def init_servo(self):
        ...
        self.move_servo(self.RIGHT_FOREARM_SERVO_ID, self.RIGHT_FOREARM_DEFAULT_ANGLE)
        self.move_servo(self.ARM_SERVO_ID, self.ARM_DEFAULT_ANGLE)
        self.move_servo(self.RIGHT_ARM_SERVO_ID, self.RIGHT_ARM_DEFAULT_ANGLE)
        ...
        self.move_servo(self.RIGHT_LOWER_LEG_SERVO_ID, self.RIGHT_LOWER_LEG_DEFAULT_ANGLE)
        return True
    ...
    def start_recording(self):
        ...
        if not self.is_recording:
            ...
            self.recording_file = f"servo_angles_{timestamp}.json"
            self.get_logger().info(f"开始记录动作到文件: {self.recording_file}")
        ...
        if self.is_recording:
            ...
            if self.recorded_data:
                try:
                    ...
                    self.get_logger().info(f"动作记录已保存到 {self.recording_file}")
                except Exception as e:
                    self.get_logger().error(f"保存动作记录失败: {str(e)}")
            else:
                self.get_logger().info("没有记录到任何动作数据")

    def record_servo_angles(self):
        """记录当前舵机角度"""
        if self.is_recording:
            current_time = time.time() - self.recording_start_time
            angles = {
                'time': current_time,
                'head': self.current_head_angle,
                'left_forearm': self.current_forearm_angle,
                'right_forearm': self.current_right_forearm_angle,
                'left_arm': self.current_arm_angle,
                'right_arm': self.current_right_arm_angle,
                'left_thigh': self.current_thigh_angle,
                'right_thigh': self.current_right_thigh_angle,
                'left_lower_leg': self.current_lower_leg_angle,
                'right_lower_leg': self.current_right_lower_leg_angle
            }
            self.recorded_data.append(angles)
    ...
    def move_servo(self, servo_id, angle):
        ...
        if servo_id == self.HEAD_SERVO_ID and abs(angle - self.current_head_angle) < 1:
            ...
            return True
        elif servo_id == self.RIGHT_FOREARM_SERVO_ID and abs(angle - self.current_right_forearm_angle) < 1:
            return True
        elif servo_id == self.ARM_SERVO_ID and abs(angle - self.current_arm_angle) < 1:
            return True
        elif servo_id == self.RIGHT_ARM_SERVO_ID and abs(angle - self.current_right_arm_angle) < 1:
            return True
            ...
            return True
        elif servo_id == self.RIGHT_LOWER_LEG_SERVO_ID and abs(angle - self.current_right_lower_leg_angle) < 1:
            return True
        ...
        if self.packetHandler.SyncWritePosEx(servo_id, position, self.SPEED, self.ACCEL):
            ...
            if result == COMM_SUCCESS:
                ...
                elif servo_id == self.RIGHT_FOREARM_SERVO_ID:
                    self.current_right_forearm_angle = angle
                    self.get_logger().info(f"右小臂舵机移动到 {angle}°")
                elif servo_id == self.ARM_SERVO_ID:
                    self.current_arm_angle = angle
                    self.get_logger().info(f"左臂舵机移动到 {angle}°")
                elif servo_id == self.RIGHT_ARM_SERVO_ID:
                    self.current_right_arm_angle = angle
                    self.get_logger().info(f"右臂舵机移动到 {angle}°")
                ...
                elif servo_id == self.RIGHT_LOWER_LEG_SERVO_ID:
                    self.current_right_lower_leg_angle = angle
                    self.get_logger().info(f"右小腿舵机移动到 {angle}°")
                
                # 记录舵机角度变化
                self.record_servo_angles()
                return True
    ...
    def calculate_right_forearm_angle(self, keypoints, confidences):
        """
        计算右小臂角度
        计算右肘指向右手腕和右肘指向右肩之间的夹角
        根据右肘指向右手腕的向量的垂直分量决定最终角度
        keypoints[6]: 右肩
        keypoints[8]: 右肘
        keypoints[10]: 右手腕
        """
        ...
        right_elbow_conf = confidences[8]      # 右肘
        right_wrist_conf = confidences[10]     # 右手腕
        
        if right_shoulder_conf < 0.4 or right_elbow_conf < 0.4 or right_wrist_conf < 0.4:
            self.get_logger().info("右小臂关键点置信度不足，使用默认角度")
            return None
        
        # 获取关键点坐标
        right_shoulder = keypoints[6]  # 右肩
        right_elbow = keypoints[8]     # 右肘（顶点）
        right_wrist = keypoints[10]    # 右手腕
        
        # 计算右肘指向右手腕的向量
        elbow_to_wrist_x = right_wrist.x - right_elbow.x
        elbow_to_wrist_y = right_wrist.y - right_elbow.y
        
        # 计算右肘指向右肩的向量
        elbow_to_shoulder_x = right_shoulder.x - right_elbow.x
        elbow_to_shoulder_y = right_shoulder.y - right_elbow.y
        ...
        self.get_logger().info(f"右肘位置: ({right_elbow.x:.1f}, {right_elbow.y:.1f}) - 顶点")
        self.get_logger().info(f"右手腕位置: ({right_wrist.x:.1f}, {right_wrist.y:.1f})")
        self.get_logger().info(f"肘->手腕向量: ({elbow_to_wrist_x:.1f}, {elbow_to_wrist_y:.1f})")
        self.get_logger().info(f"肘->肩膀向量: ({elbow_to_shoulder_x:.1f}, {elbow_to_shoulder_y:.1f})")
        self.get_logger().info(f"基础夹角: {angle_deg:.1f}°, 手腕方向: {direction}, 最终角度: {final_angle:.1f}°")
    ...
    def calculate_right_arm_angle(self, keypoints, confidences):
        """
        计算右臂角度
        """
        # 检查必要关键点的置信度
        ...
        if right_shoulder_conf < 0.4 or right_elbow_conf < 0.4 or right_hip_conf < 0.4:
            self.get_logger().info("右臂关键点置信度不足，使用默认角度")
            return None
        
        # 获取关键点坐标
        right_shoulder = keypoints[6]  # 顶点
        right_elbow = keypoints[8]     # 第一个点
        ...
        # 创建右肩正下方的参考点（垂直向下50像素，距离可以调整）
        vertical_reference = Point(right_shoulder.x, right_shoulder.y + 50)
        
        # 计算右肩-右肘连线与右肩-垂直向下连线之间的夹角
        # right_shoulder是顶点，right_elbow是第一个点，vertical_reference是第二个点
        real_arm_angle = self.calculate_angle_between_vectors(right_elbow, right_shoulder, vertical_reference)
        
        # 记录详细信息
        self.get_logger().info(f"右肩位置: ({right_shoulder.x:.1f}, {right_shoulder.y:.1f}) - 顶点")
        self.get_logger().info(f"右肘位置: ({right_elbow.x:.1f}, {right_elbow.y:.1f})")
        self.get_logger().info(f"垂直参考点: ({vertical_reference.x:.1f}, {vertical_reference.y:.1f}) - 右肩正下方")
        self.get_logger().info(f"右臂实际夹角: {real_arm_angle:.1f}° (右肩-右肘线与垂直向下线之间)")
    ...
    def calculate_right_lower_leg_angle(self, keypoints, confidences):
        """
        计算右小腿角度
        计算右膝指向右股和右膝指向右脚踝之间的夹角
        根据右膝指向右脚踝的向量的水平分量决定最终角度
        keypoints[14]: 右膝
        keypoints[12]: 右股(右胯)
        keypoints[16]: 右脚踝
        """
        # 检查必要关键点的置信度
        right_knee_conf = confidences[14]     # 右膝
        right_hip_conf = confidences[12]      # 右股(右胯)
        right_ankle_conf = confidences[16]    # 右脚踝
        
        if right_knee_conf < 0.4 or right_hip_conf < 0.4 or right_ankle_conf < 0.4:
            self.get_logger().info("右小腿关键点置信度不足，使用默认角度")
            return None
        
        # 获取关键点坐标
        right_knee = keypoints[14]   # 右膝（顶点）
        right_hip = keypoints[12]    # 右股
        right_ankle = keypoints[16]  # 右脚踝
        
        # 计算右膝指向右脚踝的向量
        knee_to_ankle_x = right_ankle.x - right_knee.x
        knee_to_ankle_y = right_ankle.y - right_knee.y
        
        # 计算右膝指向右股的向量
        knee_to_hip_x = right_hip.x - right_knee.x
        knee_to_hip_y = right_hip.y - right_knee.y
        ...
        # 记录详细信息
        self.get_logger().info(f"右膝位置: ({right_knee.x:.1f}, {right_knee.y:.1f}) - 顶点")
        self.get_logger().info(f"右股位置: ({right_hip.x:.1f}, {right_hip.y:.1f})")
        self.get_logger().info(f"右脚踝位置: ({right_ankle.x:.1f}, {right_ankle.y:.1f})")
        self.get_logger().info(f"膝->脚踝向量: ({knee_to_ankle_x:.1f}, {knee_to_ankle_y:.1f})")
        self.get_logger().info(f"膝->股向量: ({knee_to_hip_x:.1f}, {knee_to_hip_y:.1f})")
        self.get_logger().info(f"基础夹角: {angle_deg:.1f}°, 脚踝方向: {direction}, 最终角度: {final_angle:.1f}°")
    ...
    def map_real_angle_to_right_servo_angle(self, real_angle):
        """
        将实际检测到的右臂夹角映射到舵机角度
        根据要求：实际0度 -> 舵机38度，实际90度 -> 舵机128度，实际180度 -> 舵机218度
        使用线性映射：servo_angle = 38 + (real_angle / 180) * (218 - 38)
        """
        # 确保实际角度在合理范围内 (0-180度)
        real_angle = max(0, min(180, real_angle))
        
        # 线性映射：从[0, 180]映射到[38, 218]
        servo_angle = 38 + (real_angle / 180) * (218 - 38)
        
        # 确保舵机角度在允许范围内
        servo_angle = max(self.RIGHT_ARM_MIN_ANGLE, min(self.RIGHT_ARM_MAX_ANGLE, servo_angle))
        
        # 记录映射计算过程
        self.get_logger().info(f"右臂角度映射: 实际角度 {real_angle:.1f}° -> 舵机角度 {servo_angle:.1f}° (范围: {self.RIGHT_ARM_MIN_ANGLE}-{self.RIGHT_ARM_MAX_ANGLE})")
    ...
    def check_detection_timeout(self):
        ...
        if current_time - self.last_detection_time > self.detection_timeout:
            ...
            if self.current_right_forearm_angle != self.RIGHT_FOREARM_DEFAULT_ANGLE:
                self.get_logger().info("长时间未检测到人，右小臂回到默认位置(180°)")
                self.move_servo(self.RIGHT_FOREARM_SERVO_ID, self.RIGHT_FOREARM_DEFAULT_ANGLE)
                self.last_real_right_forearm_angle = 180  # 重置为默认值
            ...
            if self.current_right_arm_angle != self.RIGHT_ARM_DEFAULT_ANGLE:
                self.get_logger().info("长时间未检测到人，右臂回到默认位置(128°)")
                self.move_servo(self.RIGHT_ARM_SERVO_ID, self.RIGHT_ARM_DEFAULT_ANGLE)
                self.last_real_right_arm_angle = 90  # 重置为默认值
            ...
            if self.current_right_lower_leg_angle != self.RIGHT_LOWER_LEG_DEFAULT_ANGLE:
                self.get_logger().info("长时间未检测到人，右小腿回到默认位置(180°)")
                self.move_servo(self.RIGHT_LOWER_LEG_SERVO_ID, self.RIGHT_LOWER_LEG_DEFAULT_ANGLE)
                self.last_real_right_lower_leg_angle = 180  # 重置为默认值
            ...
            if target.type == "person":
                for point_group in target.points:
                    if point_group.type == "body_kps" and len(point_group.point) >= 18:
                        ...
                        has_right_forearm_points = (
                            confidences[6] > 0.3 and    # 右肩
                            confidences[8] > 0.3 and    # 右肘
                            confidences[10] > 0.3       # 右手腕
                        )
                        ...
                        has_right_arm_points = (
                            confidences[6] > 0.3 and    # 右肩
                            confidences[8] > 0.3 and    # 右肘
                            confidences[12] > 0.3       # 右胯
                        )
                        ...
                        has_right_lower_leg_points = (
                            confidences[14] > 0.3 and   # 右膝
                            confidences[12] > 0.3 and   # 右股(右胯)
                            confidences[16] > 0.3       # 右脚踝
                        )
                        
                        if (has_head_points or has_forearm_points or has_right_forearm_points or 
                            has_arm_points or has_right_arm_points or 
                            has_left_thigh_points or has_right_thigh_points or 
                            has_lower_leg_points or has_right_lower_leg_points):
                            ...
                            # 控制右小臂舵机
                            if has_right_forearm_points:
                                real_right_forearm_angle = self.calculate_right_forearm_angle(keypoints, confidences)
                                
                                if real_right_forearm_angle is not None:
                                    # 检查角度变化是否超过阈值
                                    angle_diff = abs(real_right_forearm_angle - self.last_real_right_forearm_angle)
                                    
                                    if angle_diff >= self.RIGHT_FOREARM_ANGLE_THRESHOLD:
                                        # 角度变化超过阈值，执行舵机控制
                                        servo_angle = int(real_right_forearm_angle)
                                        self.get_logger().info(f"右小臂实际角度: {real_right_forearm_angle:.1f}° -> 舵机角度: {servo_angle}°")
                                        self.get_logger().info(f"角度变化: {angle_diff:.1f}° >= {self.RIGHT_FOREARM_ANGLE_THRESHOLD}°，执行舵机控制")
                                        self.move_servo(self.RIGHT_FOREARM_SERVO_ID, servo_angle)
                                        self.last_real_right_forearm_angle = real_right_forearm_angle
                                    else:
                                        self.get_logger().info(f"右小臂角度变化不足: {angle_diff:.1f}° < {self.RIGHT_FOREARM_ANGLE_THRESHOLD}°，保持当前舵机位置")
                            ...
                            # 控制右臂舵机
                            if has_right_arm_points:
                                real_right_arm_angle = self.calculate_right_arm_angle(keypoints, confidences)
                                
                                if real_right_arm_angle is not None:
                                    # 检查角度变化是否超过阈值
                                    angle_diff = abs(real_right_arm_angle - self.last_real_right_arm_angle)
                                    
                                    if angle_diff >= self.RIGHT_ARM_ANGLE_THRESHOLD:
                                        # 角度变化超过阈值，执行舵机控制
                                        servo_angle = self.map_real_angle_to_right_servo_angle(real_right_arm_angle)
                                        self.get_logger().info(f"右臂实际角度: {real_right_arm_angle:.1f}° -> 舵机角度: {servo_angle}°")
                                        self.get_logger().info(f"角度变化: {angle_diff:.1f}° >= {self.RIGHT_ARM_ANGLE_THRESHOLD}°，执行舵机控制")
                                        self.move_servo(self.RIGHT_ARM_SERVO_ID, servo_angle)
                                        self.last_real_right_arm_angle = real_right_arm_angle
                                    else:
                                        self.get_logger().info(f"右臂角度变化不足: {angle_diff:.1f}° < {self.RIGHT_ARM_ANGLE_THRESHOLD}°，保持当前舵机位置")
                            ...
                            # 控制右小腿舵机
                            if has_right_lower_leg_points:
                                real_right_lower_leg_angle = self.calculate_right_lower_leg_angle(keypoints, confidences)
                                
                                if real_right_lower_leg_angle is not None:
                                    # 检查角度变化是否超过阈值
                                    angle_diff = abs(real_right_lower_leg_angle - self.last_real_right_lower_leg_angle)
                                    
                                    if angle_diff >= self.RIGHT_LOWER_LEG_ANGLE_THRESHOLD:
                                        # 角度变化超过阈值，执行舵机控制
                                        servo_angle = int(real_right_lower_leg_angle)
                                        self.get_logger().info(f"右小腿实际角度: {real_right_lower_leg_angle:.1f}° -> 舵机角度: {servo_angle}°")
                                        self.get_logger().info(f"角度变化: {angle_diff:.1f}° >= {self.RIGHT_LOWER_LEG_ANGLE_THRESHOLD}°，执行舵机控制")
                                        self.move_servo(self.RIGHT_LOWER_LEG_SERVO_ID, servo_angle)
                                        self.last_real_right_lower_leg_angle = real_right_lower_leg_angle
                                    else:
                                        self.get_logger().info(f"角度变化不足: {angle_diff:.1f}° < {self.RIGHT_LOWER_LEG_ANGLE_THRESHOLD}°，保持当前舵机位置")
...
def main(args=None):
    ...
    try:
        ...
        controller.move_servo(controller.RIGHT_FOREARM_SERVO_ID, controller.RIGHT_FOREARM_DEFAULT_ANGLE)
        controller.move_servo(controller.ARM_SERVO_ID, controller.ARM_DEFAULT_ANGLE)
        controller.move_servo(controller.RIGHT_ARM_SERVO_ID, controller.RIGHT_ARM_DEFAULT_ANGLE)
        ...
        controller.move_servo(controller.RIGHT_LOWER_LEG_SERVO_ID, controller.RIGHT_LOWER_LEG_DEFAULT_ANGLE)
